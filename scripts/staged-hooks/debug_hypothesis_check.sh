...
# Writes to /tmp so we can see what values the check actually gets
{
  ...
  echo "state_file_exists=$(test -f "${PROJECT_DIR}/.claude/state/task-workflow/current-task.json" && echo YES || echo NO)"
  if [[ -f "${PROJECT_DIR}/.claude/state/task-workflow/current-task.json" ]]; then
    echo "hypothesis=$(jq -r '.hypothesis_stated' "${PROJECT_DIR}/.claude/state/task-workflow/current-task.json")"
  fi
