  ...
  return (
    <div className="px-6 py-16">
      <div className="mx-auto max-w-3xl">
        ...
        <h1 className="text-4xl font-extrabold tracking-tight text-slate-900">Quickstart</h1>
        ...
        <div className="prose mt-10 max-w-none">
          ...
          <Step number={1} title="Create a workspace and get an API key">
            <p className="text-slate-600">
              ...
              Copy the key — it starts with <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-sm">tbx_</code> and is shown only once.
            </p>
      ...
      "headers": {
        "Authorization": "Bearer tbx_YOUR_API_KEY"
            ...
            <p className="mt-4 text-sm text-slate-500">
              Replace <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-xs">tbx_YOUR_API_KEY</code> with the key from step 1.
            </p>
          </Step>
          ...
          <Step number={3} title="Create your first project">
            <p className="text-slate-600">
              In your MCP client, ask your agent to call the <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-sm">create_project</code> tool:
            </p>
 ...
 Then capture this thought: 'Thoughtbox is connected and working.'"`}</CodeBlock>
        ...
        <div className="mt-12 rounded-xl border border-brand-100 bg-brand-50 p-6">
          ...
          <ul className="mt-3 flex flex-col gap-2 text-sm text-slate-600">
            <li>
              <Link href="/docs" className="text-brand-600 hover:underline">Explore the full docs</Link>
              {' '}— learn about knowledge graphs, semantic search, and run analysis.
            </li>
            <li>
              <Link href="/pricing" className="text-brand-600 hover:underline">Check the pricing page</Link>
              {' '}— upgrade to Pro for higher limits and 1-year history.
            </li>
            <li>
              <Link href="/support" className="text-brand-600 hover:underline">Contact support</Link>
...
function CodeBlock({ children }: { children: string }) {
  return (
    <pre className="mt-4 overflow-x-auto rounded-xl bg-slate-900 p-5 font-mono text-sm leading-relaxed text-slate-200">
      <code>{children}</code>
    </pre>
  )
}
