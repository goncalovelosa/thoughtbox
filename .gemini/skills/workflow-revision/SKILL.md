...
1. Create a tracker item for the fix: the selected issue create operation
...
- Close any fix selected tracker items in this stage
...
D. Abandon workflow (close tracker item, delete branch)
...
- Each fix sub-agent gets its own tracker item (1 tracker item = 1 fix = 1 commit after review passes)
