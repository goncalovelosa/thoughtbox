...
2. Create a tracker item for the workflow if one doesn't exist
...
```json
{
  ...
  "tracker itemId": "<tracker item-id>",
...
### Task
- Tracker item: the selected tracker-XXX
...
Save summaries to: `.adr/staging/<NNN>-<name>-summary-<tracker item-id>.md`

## Operational Rules

1. **1 tracker item = 1 sub-agent = 1 commit**: Each sub-agent's unit of work is exactly one commit, made after review validates the work — not during implementation.
...
```
WORKFLOW: <title>
Branch: <branch> | Tracker item: <tracker itemId>
Started: <startedAt>
