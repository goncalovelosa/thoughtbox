...
2. **the selected tracker backlog**: the selected tracker readiness view — issues with no blockers, ready to work
3. **In-progress work**: the selected issue list — work started but not finished
...
| **IN_PROGRESS_ITEMS** | the selected issue list returns items |
| **READY_ITEMS** | the selected tracker readiness view returns items |
...
| UNFIXED_FINDINGS | Pick one finding, create a the selected tracker issue for it, work it |
...
4. If verdict = "proceed" → create a the selected tracker issue and loop back to READY_ITEMS
...
```json
{
  ...
  "budget_usd": 5.00,
  "actions": [
    { "iteration": 1, "situation": "READY_ITEMS", "action": "Claimed the selected tracker-abc, dispatched triage-fix", "cost_usd": 0.50 },
    { "iteration": 2, "situation": "IN_PROGRESS_ITEMS", "action": "Completed the selected tracker-abc, committed fix", "cost_usd": 0.80 },
    { "iteration": 3, "situation": "BACKLOG_EMPTY", "action": "QD exploration: archetype=exploratory, niche=(1,2,3,1,2)", "cost_usd": 0.40 },
    { "iteration": 4, "situation": "READY_ITEMS", "action": "Working on QD-discovered the selected tracker-def", "cost_usd": 0.60 }
  ],
...
- All the selected tracker are closed AND no adversarial findings are unfixed AND QD exploration has had 3+ consecutive misses
...
- Do NOT create more than 3 new the selected tracker issues per iteration. Creating work is not doing work.
...
2. the selected issue list — close or revert any in-progress items
