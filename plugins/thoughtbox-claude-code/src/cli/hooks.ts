...
export async function runCaptureUserTurn(
  args: string[],
  runtime: HookRuntime,
): Promise<number> {
  ...
  if (!isSyncDelivery(args, runtime.env)) {
    try {
      ...
      runtime.writeStderr(
        `thoughtbox capture-user-turn: detach failed (${describe(error)}); turn not captured`,
      );
    }
    return 0;
  }
  ...
  if (!connection) {
    runtime.writeStderr(
      'thoughtbox capture-user-turn: no Thoughtbox endpoint configured; turn not captured',
    );
    return 0;
  }
  ...
  try {
    ...
    runtime.writeStderr(
      `thoughtbox capture-user-turn: delivery failed (${describe(error)}); turn not captured`,
    );
  }
  return 0;
}
...
export async function runSurfaceDecisions(
  args: string[],
  runtime: HookRuntime,
): Promise<number> {
  const payload = await readHookPayload(args, runtime);
  ...
  try {
    const url =
      `${connection.baseUrl}/drift/session-decisions` +
  ...
  const lines = decisions.map((decision, index) => {
    const label =
      decision.thoughtType === 'assumption_update'
        ? 'Correction'
        : 'Decision';
    const text =
      decision.thought.length > MAX_DECISION_CHARS
...
function isDecisionItem(value: unknown): value is DecisionItem {
  ...
  return (
    typeof item.thoughtNumber === 'number' &&
    typeof item.thoughtType === 'string' &&
    typeof item.thought === 'string'
  );
}

function reminderLimit(env: NodeJS.ProcessEnv): number {
  const parsed = Number.parseInt(env.THOUGHTBOX_DRIFT_REMINDER_LIMIT ?? '', 10);
  if (!Number.isFinite(parsed) || parsed < 1) return DEFAULT_REMINDER_LIMIT;
 ...
 */
const DEFAULT_CORRECTION_PATTERNS: RegExp[] = [
...
export function matchesCorrectionLanguage(
  prompt: string,
...
export async function runPromoteToDecision(
  args: string[],
  runtime: HookRuntime,
): Promise<number> {
  const payload = await readHookPayload(args, runtime);
  ...
  if (!matchesCorrectionLanguage(prompt, runtime.env)) return 0;
  ...
  if (!isSyncDelivery(args, runtime.env)) {
    try {
      ...
      runtime.writeStderr(
        `thoughtbox promote-to-decision: detach failed (${describe(error)})`,
      );
    }
    return 0;
  }
  ...
  try {
    ...
    runtime.writeStderr(
      `thoughtbox promote-to-decision: delivery failed (${describe(error)})`,
    );
  }
  return 0;
}
...
// ---------------------------------------------------------------------------
// audit-response (spec §5) — local pattern scan, no network
// ---------------------------------------------------------------------------
...
export function findUnverifiedAssertions(
  turn: TurnRecord,
  env: NodeJS.ProcessEnv,
): string[] {
  const patterns = [
    ...DEFAULT_ASSERTION_PATTERNS,
    ...parseRegexListEnv(env.THOUGHTBOX_AUDIT_PATTERNS).map(
  ...
  for (const pattern of patterns) {
    ...
    for (const match of turn.responseText.matchAll(pattern)) {
      ...
      if (target && looksLikeRepoTarget(target)) targets.add(target);
    }
  }
...
function looksLikeRepoTarget(target: string): boolean {
  if (target.length < 3 || target.length > 200) return false;
  if (!/[./]/.test(target)) return false;
  if (/^(e\.g|i\.e|etc|vs)\.?$/i.test(target)) return false;
 ...
 */
export function parseLastTurn(transcript: string): TurnRecord {
  ...
  const toolUses: TurnRecord['toolUses'] = [];
...
export async function runAuditResponse(
  args: string[],
  runtime: HookRuntime,
): Promise<number> {
  const payload = await readHookPayload(args, runtime);
  ...
  const reason = unverified
    .map(
      (target) =>
        `Assertion about ${target} was not verified in this turn. ` +
...
export async function runHookCommand(
  ...
  switch (hookName) {
    ...
    default:
      runtime.writeStderr(
        `error: unknown hook "${hookName ?? ''}" (expected capture-user-turn, surface-decisions, promote-to-decision, or audit-response)`,
      );
