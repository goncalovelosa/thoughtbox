...
export function hookTimeoutMs(env: NodeJS.ProcessEnv): number {
 ...
 */
export async function resolveHookConnection(
  runtime: Pick<HookRuntime, 'cwd' | 'env'>,
): Promise<HookConnection | null> {
  const { env } = runtime;
  const projectDir = env.CLAUDE_PROJECT_DIR ?? runtime.cwd;

  let baseUrl: string | null =
    env.THOUGHTBOX_URL ?? env.OTEL_EXPORTER_OTLP_ENDPOINT ?? null;
  let apiKey: string | null = env.THOUGHTBOX_API_KEY ?? null;

  if (!apiKey && env.OTEL_EXPORTER_OTLP_HEADERS) {
    const match = env.OTEL_EXPORTER_OTLP_HEADERS.match(
...
export function authHeaders(connection: HookConnection): Record<string, string> {
  return {
 ...
 */
export function isSyncDelivery(args: string[], env: NodeJS.ProcessEnv): boolean {
 ...
 */
export async function spawnDetachedSelf(
  hookName: string,
  ...
  const payloadPath = path.join(
    os.tmpdir(),
    `thoughtbox-hook-${randomUUID()}.json`,
  );
  ...
  const child = spawn(
    process.execPath,
    [binPath, 'hook', hookName, '--sync', '--payload-file', payloadPath],
...
export function parseRegexListEnv(value: string | undefined): RegExp[] {
  if (!value) return [];
  try {
