...
# DEBUG: did we reach the logging section?
echo "REACHED LOGGING $(date)" >> /tmp/hypothesis-debug.txt 2>&1
...
echo "$log_data" | jq --argjson new "$input_json" '. += [$new]' > logs/pre_tool_use.json \
  || echo "[$(date -u +%FT%TZ)] pre_tool_use: jq audit log append failed" >&2
...
# DEBUG: hypothesis check
{
  echo "=== HYPOTHESIS DEBUG $(date) ==="
  echo "tool_name=$tool_name"
  _dbg_fp=$(echo "$tool_input" | jq -r '.file_path // ""')
  echo "_fp=$_dbg_fp"
  echo "matches_src=$([[ "$_dbg_fp" == */src/* ]] && echo YES || echo NO)"
  echo "bead_file=$(test -f "${PROJECT_DIR}/.claude/state/bead-workflow/current-bead.json" && echo EXISTS || echo MISSING)"
