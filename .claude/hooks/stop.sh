...
load_env_var_from_file_if_unset() {
    local key="$1"
...
load_env_var_from_file_if_unset "TRACE_TO_LANGSMITH"
load_env_var_from_file_if_unset "LANGSMITH_API_KEY"
load_env_var_from_file_if_unset "CC_LANGSMITH_API_KEY"
load_env_var_from_file_if_unset "LANGSMITH_PROJECT"
load_env_var_from_file_if_unset "CC_LANGSMITH_PROJECT"

# If a key exists and trace flag is absent, default tracing on.
if [ -z "${TRACE_TO_LANGSMITH:-}" ] && { [ -n "${CC_LANGSMITH_API_KEY:-}" ] || [ -n "${LANGSMITH_API_KEY:-}" ]; }; then
    TRACE_TO_LANGSMITH="true"
    export TRACE_TO_LANGSMITH
fi
...
PROJECT="${CC_LANGSMITH_PROJECT:-${LANGSMITH_PROJECT:-claude-code}}"
...
# Load state (missing or empty file => {})
load_state() {
